var DBName = DB_scanListName;
var barcodelist = new Array();


function load(){
    // ローカルストレージが使用できるか判断する
    if ( ls = localStorage.getItem(DBName)){
        if ( ls != null){
            // データベースを読めるようにする
            barcodelist = JSON.parse(ls);
...
function save(){
    localStorage.setItem( DBName, JSON.stringify( barcodelist));
    show();
    
}

function entry_db( codeno){
    if (codeno == null ){
        window.alert( "登録するデータがありません");
...
function shougou() {
    console.log("hit shougou key.");
    window.location.href="./print-report.html";
